...
- Q: What cache lifetime/scope should season badge caching use? -> A: In-memory cache only for current app session (cleared on refresh).
- Q: How should the UI behave when a season badge request fails? -> A: Keep previous badge (if any) and show inline error for current request.
- Q: What search matching behavior should be used for league filtering? -> A: Case-insensitive substring match on league name only.
- Q: What search input update strategy should be used? -> A: Debounce search input by 300 ms.
- Q: How should concurrent league-click badge requests be resolved? -> A: Latest click wins; older in-flight responses are ignored.
...
4. **Given** the user clicks multiple league items quickly, **When** badge requests complete out of order, **Then** only the most recently selected league's result is rendered and earlier responses are ignored.
...
- **FR-006**: System MUST provide a text search input that performs case-insensitive substring filtering on league name only.
...
- **FR-016**: System MUST debounce text-search driven filter updates by 300 ms.
...
- **FR-012**: System MUST cache season badge responses by league identifier in memory for the current app session only (cleared on page refresh) and reuse cached results for repeat selections.
- **FR-013**: System MUST provide informative UI states for loading, empty results, and request failures.
- **FR-015**: System MUST keep the previously shown badge visible (if one exists) and display an inline error message when a newly selected league's badge request fails.
- **FR-017**: System MUST treat rapid league selections with a latest-click-wins policy, rendering only the most recent selection's badge result and ignoring stale in-flight responses.
...
- **Badge Cache Entry**: Represents an in-memory cached season badge response keyed by league identifier for the current app session only.
